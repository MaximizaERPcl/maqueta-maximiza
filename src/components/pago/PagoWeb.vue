      ...
      >
        ...
        >
            ...
            <v-col cols="12" md="8">
              ...
              >
              <template
              v-slot:item.vigente="{ item }"
            >
              Abono Pactado
            </template>
            <template
                  v-slot:item.abono_pactado="{ item }"
                >
              ...
              >
                <template v-slot:top>
                  ...
                  height="35"
                  >
                    <v-toolbar-title class="flex text-center cabecera">Gimnasio</v-toolbar-title>
                  </v-system-bar>
                </template>
              </v-data-table>
              <v-select
                ...
                :style="{width: $vuetify.breakpoint.mdAndUp? '18rem' : '100%'}"
                dense
                item-text="nombre"
                label="Renovar Plan"
              ...
              <template
                ...
                <template v-slot:top>
                  ...
                  height="35"
                  >
                    <v-toolbar-title class="flex text-center cabecera">Comisión por useo de tarjeta de crédito</v-toolbar-title>
                  </v-system-bar>
                </template>
              </v-data-table>
            </v-col>
            <v-col cols="12" md="4">
              <v-card outlined flat>
                ...
                <v-alert
                  ...
                  class="mx-1 mb-1"
                >
                Recuerde realizar pago con tarjeta de débito, de lo contrario se cobrará otro cargo por servicio.
                </v-alert>
              </v-card>
            </v-col>
            </v-row>
    ...
    data() {
      ...
      methods: {
        sumAll(){
          ...
          let cargo = (tempSuma * parseFloat(this.pagos.tasa_web[0].tasa_tbk))/100
        ...
        seleccionItem({value, item}) {
          if(value){
            ...
            this.selected = this.selected.filter(pago => 
              pago.id != item.id &&
              (pago.credito === item.credito &&
              parseInt(pago.n_cuota) < parseInt(item.n_cuota)) || (pago.credito !== item.credito))
          }
        }
      },
      computed: {
      ...
      watch: {
        ...
        selectedGym(newVal, oldVal){
          if(newVal){
            this.totales[1].monto = parseInt(newVal.valor);
          } 
          else
            this.totales[1].monto = 0;
          this.sumAll();
        }
      },
  };
